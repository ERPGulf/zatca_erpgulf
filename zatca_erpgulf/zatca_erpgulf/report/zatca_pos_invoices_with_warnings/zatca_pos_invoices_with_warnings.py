...
def execute(filters=None):
    ...
    invoices = frappe.db.sql(
        ...
        WHERE {where_clause}
          AND pi.custom_zatca_full_response IS NOT NULL
          AND pi.custom_zatca_full_response LIKE '%%"warningMessages"%%'
        """,
        query_filters,
        as_dict=True,
    )
    ...
    for inv in invoices:
        try:
            ...
            frappe.log_error(
                title="ZATCA POS Invoice Warning Report Error",
                message=f"POS Invoice: {inv.name}\n{str(e)}",
            )
